...
import { normalizePauseShortcodes } from "./meditationScanner";
...
• Füge NIEMALS eigene [PAUSE] Tags hinzu - diese werden VOR der Verarbeitung generiert!

PAUSE-TAGS [PAUSE Xs] SIND HEILIG:
• Vorhandene [PAUSE 840s] oder ähnliche Tags NIEMALS ändern oder entfernen!
• Diese Tags wurden bereits korrekt berechnet und eingefügt
• Beispiel: "[PAUSE 840s]" → bleibt EXAKT so
...
function protectStageDirections(text: string): { protectedText: string; originalLines: string[] } {
    ...
    // ============================================================
    // v2.4.7 SIMPLIFIED PROTECTION
    // ============================================================
    // No more aggressive guessing. We protect:
    // 1. Lines with explicit keywords (PAUSE, STILLE, NACHSPÜREN)
    // 2. Lines containing [PAUSE Xs] tags (from normalizePauseShortcodes)
    // 3. Stage directions in brackets

    // Pattern 1: Classic keywords at line start
    const primaryPattern = /^(?:(?:KURZE|LANGE|KLEINE|GROSSE)\s+)?(?:PAUSE|STILLE|NACHSPÜREN)\b/i;

    // Pattern 2: Lines containing [PAUSE Xs] system tags
    // These were generated by normalizePauseShortcodes() from (14 Min) → [PAUSE 840s]
    const pauseTagPattern = /\[PAUSE\s+\d+(?:\.\d+)?s\]/i;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (primaryPattern.test(trimmedLine) ||
            pauseTagPattern.test(trimmedLine) ||
...
export async function processChunkWithWatchdog(
    ...
    // ============================================================
    // v2.4.7: SHORTCODE NORMALIZATION (FIRST STEP)
    // ============================================================
    // Convert user-friendly shortcodes to system tags BEFORE protection
    // Example: "(14 Min)" → "[PAUSE 840s]"
    let normalizedChunk = chunk;
    if (isMeditationMode) {
        normalizedChunk = normalizePauseShortcodes(chunk);
    }

    // MEDITATION MODE: Protect stage direction lines before sending to AI
    let textToProcess = normalizedChunk;
    let protectedLines: string[] = [];

    if (isMeditationMode) {
        const protection = protectStageDirections(normalizedChunk);
